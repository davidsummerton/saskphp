...
class Database
{
     ...
     */
    public function find($tableName, $columnName, $value, $numRows = null)
    {
        //$this->where($columnName, $value);
        $this->_query = "SELECT * FROM $tableName WHERE $columnName = '$value'";
        $stmt = $this->_buildQuery($numRows);
        $stmt->execute();
        $this->reset();
    ...
    public function findFirst($tableName, $columnName, $value)
    {
        $result = $this->find($tableName, $columnName, $value);
        if (!empty($result)) {
            return $result[0];
        } else {
            return false;
     ...
     * @return boolean
     */
    public function update($tableName, $columnName, $columnValue, $tableData)
    {
     ...
     */
    public function delete($tableName, $columnName, $value, $numRows = null)
    {
